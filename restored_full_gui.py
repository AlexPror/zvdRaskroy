...
class RestoredFullGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Раскрой Деталей v2.0 - ВОССТАНОВЛЕННАЯ ПОЛНАЯ ВЕРСИЯ")
    ...
    def optimize_nesting(self):
        ...
        def optimization_thread():
            try:
                ...
                # Используем правильный способ получения результатов из rectpack
                bins = []
                for bin in packer:
                    bins.append(bin)
                ...
                if bins:
                    ...
                    for bin_idx, bin_data in enumerate(bins):
                        ...
                        # ИСПРАВЛЯЕМ: rectpack возвращает (x, y, w, h), а не (x, y, w, h, name)
                        for rect in bin_data:
                            x, y, w, h = rect  # Только 4 элемента
                            # Находим имя детали по размеру из исходных данных
                            name = self.find_part_name_by_size(w, h)
                            part = {
                                'name': name,
                                'x': x,
                                'y': y,
    ...
    def find_part_name_by_size(self, width, height):
        """Найти имя детали по размеру"""
        # Ищем деталь с похожим размером (с учетом зазора)
        for file_data in self.files_data:
            if abs(file_data['width'] + 5 - width) < 1 and abs(file_data['height'] + 5 - height) < 1:
                return file_data['name']
        # Если не найдено, возвращаем общее имя
        return f"Деталь_{width}x{height}"
